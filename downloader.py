...
from task_manager import Message
...
class Downloader():
    ...
    def run(self, p_conn, c_conn):
        running = True
        self.log.info("Running task : {}".format(current_process().name))

        pipe_child_msgs = []
        pipe_child_msgs.append(c_conn)
        message = Message()
        ...
        while running:
            ...
            message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_START_TIMER)
            ...
            if (ans == b'\x5d'):
                ...
                message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_UNKNOWN_MODE)
                running = False
                sys.exit(1)
            
            if (uploading_state == 0) :
                self.log.info('In application mode, communication checking...')
                message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_APPLICATION_MODE)
                ...
                while (count < 4):
                    ...
                    message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_APP_MODE_COMM_FAIL)
                    running = False
                    sys.exit(1)

                # change to bootloader mode
                self.log.info("Switching to sbsl mode ...")
                message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_SWITCH_TO_SBSL)
                count = 0
                while (count < 4):
                    ...
                    self.log.info('msg: {} --> {}'.format(msg.hex(), ans.hex()))
                    message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_CANNOT_ENTER_SBSL_MODE)
                    running = False
                    sys.exit(1)
            ...
            message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_PARSE_FILE)
            parse_mode = 0
            firmware_cmd = []
            parameter_cmd = []
            script_cmd = []
            ...
            if uploading_state == 1:
                ...
                if en_autobaudrate == 1:
                    ...
                    message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_BOOTLOADER_MODE)
                    ...
                    if (ans != b'\x5d'):
                        ...
                        self.log.info('msg: {} --> {}'.format(msg.hex(), ans.hex()))
                        message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_SBSL_AUTOBAUD_FAIL)
                        sys.exit(1)
                ...
                message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_FBSL_ID, payload=sbsl_id.hex())

                # firmware uploading to device
                self.log.info('Firmware uploading...')
                message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_START_UPLOADING_FIRMWARE)
                ...
                for msg in firmware_cmd:
                    ...
                    if msg[1] == 0x20:
                        ...
                        if ans[-2:] != b'\x90\x00':
                            ...
                            self.log.info('msg: {} --> {}'.format(msg.hex(), ans.hex()))
                            message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_FIRMWARE_UPLOAD)
                            running = False
                            sys.exit(1)
                    elif msg[1] == 0x21:
                        ans = self.ser.read(3) ###
                    ...
                    progress = int((msg_count/msg_count_max)*100)
                    message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_UPLOAD_PROGRESS, payload=progress)
            ...
            if uploading_state == 2:
                ...
                if en_autobaudrate == 1:
                    ...
                    message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_CONFIG_MODE)
                    ...
                    if (ans != b'\xcd'):
                        ...
                        self.log.info('msg: {} --> {}'.format(msg.hex(), ans.hex()))
                        message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_CONFIG_MODE_AUTOBAUD_FAIL)
                        running = False
                        sys.exit(1)

                # parameter uploading to device
                self.log.info('Parameter uploading...')
                message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_PARAMETER_UPLOAD)
                ...
                for msg in parameter_cmd:
                    ...
                    if ans[-2:] != b'\x90\x00':
                        ...
                        self.log.info('msg: {} --> {}'.format(msg.hex(), ans.hex()))
                        message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_PARAMETER_UPLOAD)
                        running = False
                        sys.exit(1)
                    time.sleep(.100)
                ...
                message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_SCRIPT_UPLOAD)
                ...
                for msg in script_cmd:
                    ...
                    if ans[-2:] != b'\x90\x00':
                        ...
                        self.log.info('msg: {} --> {}'.format(msg.hex(), ans.hex()))
                        message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_SCRIPT_UPLOAD)
                        sys.exit(1)
                    time.sleep(.100)
                ...
                if ans != b'\x90\x00\xff':
                    ...
                    self.log.info('msg: {} --> {}'.format(msg.hex(), ans.hex()))
                    message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_ERROR, payload=c.ERROR_SWITCH_APP_MODE)
                    running = False
                    sys.exit(1)

                self.log.info('Finish uploading...')
                self.log.info('Success')
                message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_FINISH_UPLOAD)

            end_time = datetime.now()
            message.send(pipe_id=c_conn, msg=c.MESSAGE_DOWNLOADER_END_TIMER)
